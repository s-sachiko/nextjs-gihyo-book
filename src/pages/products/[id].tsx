...
const ProductPage: NextPage<ProductPageProps> = ({ id, product: initial }: ProductPageProps) => {
  ...
  return (
    <Layout>
      <Flex paddingTop={2} paddingBottom={2} paddingLeft={{ base: 2, md: 0 }} paddingRight={{ base: 2, md: 0 }} justifyContent="center" flexDirection={{ base: "column", md: "row" }}>
        <Box>
          <Breadcrumb>
            <BreadcrumbItem>
              <Link href="/">トップ</Link>
            ...
            <BreadcrumbItem>
              <Link href={`/search/${product.category}`}>{categoryNameDict[product.category as Category]}</Link>
            </BreadcrumbItem>
          ...
          <Box paddingTop={1}>
            ...
            <Link href={`/users/${product.owner.id}`}>
              {/* ユーザープロファイル */}
              <UserProfile variant="small" username={product.owner.username} profileImageUrl={product.owner.profileImageUrl} numberOfProducts={100} />
            </Link>
          </Box>
        </Box>
